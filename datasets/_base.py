...
def load_yaz(include_date=False,one_hot_encoding=False,label_encoding=False,return_X_y=False):
    ...
    long_format =True 
    if long_format:
        df=pd.concat([data,target],axis=1).melt(id_vars=data.columns,var_name='product',value_name='demand')
    ...
    if one_hot_encoding:
        cols=[c for c in ['weekday','month','year','product'] if c in data.columns]
    ...
    n_targets=1 if long_format else target.shape[1]

    if return_X_y:
        return data,target

    return Bunch(data=data,target=target,n_features=n_features,n_targets=n_targets,DESCR=fdescr,data_filename=data_filename,target_filename=target_filename)
